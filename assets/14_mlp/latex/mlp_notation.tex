...
\tikzstyle{neuron}=[circle,minimum size=17pt,inner sep=0pt, fill = brown!60, draw = black]
...
\begin{tikzpicture}[shorten >=1pt,draw=black,thick, node distance=\layersep, >= stealth]
    ...
    \begin{scope}
        ...
        % %%%%%%%%%%%%%% (l-1) LAYER %%%%%%%%%%%%%%%%%%%%%%
        \draw [white, fill = brown!15] (-\pad, -4 - 2*\pad) rectangle (\pad, 2*\pad);
        \node at (0, -4.5 - 2*\pad) {$\ba^{(l-1)}$};
        \node at (0, 1.5+2.5*\pad) {$(l-1)^{\text{th}}$ layer};

        \def\y{0}
        \node[neuron, fill = brown!10] (I-\y) at (0,-2*\y + 2) {1};

        \def\y{1}
        \node[neuron, label = below:$a^{(l-1)}_\y$] (I-\y) at (0,-2*\y + 2) {};
        \node[scale = 1.5]  at (0,-2*\y + 1) {$\vdots$};

        \def\y{2}
        \node[neuron, label = below:$a^{(l-1)}_j$] (I-\y) at (0,-2*\y + 2) {};
        \node[scale = 1.5]  at (0,-2*\y + 1) {$\vdots$};

        \def\y{3}
        \node[neuron, label = below:$a^{(l-1)}_{d^{(l-1)}}$] (I-\y) at (0,-2*\y + 2) {};

        %%%%%%%%%%%%%%%% (l)) LAYER %%%%%%%%%%%%%%%%%%%%%%
        \node [scale = 1.5, anchor = west] at (1.25*\layersep, -1.5) {$\mathbf{W}^{(l)} \in \R^{d^{(l)}\times d^{(l-1)}}$};
        \node [scale = 1.5, anchor = west] at (1.25*\layersep, -2.5) {$\mathbf{b}^{(l)} \in \R^{d^{(l)}\times1}$};
        \node [scale = 1.5, anchor = west] at (1.25*\layersep, -3.5) {$z_i^{(l)} = \mathbf{w}_i^{(l)T}\ba^{(l-1)} + b_i^{(l)}$};
        ...
        \draw [white, fill = brown!15] (-\pad + \layersep, -5 - 2*\pad)
                            rectangle ( \pad + \layersep , 1+2*\pad);
        \node at (\layersep, -5.5 - 2*\pad) {$\ba^{(l)}$};

        % \foreach \name / \y in {1, 2, 3, 4}
        \def\y{1}
        \path[yshift=3cm] node[neuron, label = below:{$a^{(l)}_\y$}] (H-\y) at (\layersep,-2*\y cm) {};
        ...
        \def\y{3}
        \path[yshift=3cm] node[neuron, label = below:{$a^{(l)}_i$}] (H-\y) at (\layersep,-2*\y cm) {};
        \node[scale = 1.5]  at (\layersep,-2*\y + 1.8) {$\vdots$};

        \def\y{4}
        \path[yshift=3cm] node[neuron, label = below:{$a^{(l)}_{d^{(l)}}$}] (H-\y) at (\layersep,-2*\y cm) {};

        \foreach \src in {0, 1, 2, 3}
            \foreach \dst in {1, 2, 3, 4}
                \draw [->, draw = gray!40] (I-\src) -- (H-\dst);
        % \node[hidden neuron, opacity = .5, label = below:{$a^{(1)}_0$}] (H-0) at (\layersep,0) {1};

        \draw [thick, ->](I-2) -- node[midway, fill = white, ] {$w^{(l)}_{ij}$} (H-3);
        \draw [thick, ->](I-0) -- node[midway, fill = white, ] {$b^{(l)}_{i}$} (H-3);

...
\begin{tikzpicture}[shorten >=1pt,draw=black,thick, node distance=\layersep, >= stealth]
   ...
   \begin{scope}[xshift = 7cm]
    ...
    \node [text width= 4cm, anchor = west] at (-1, \offset - 4*\d) {$\mathbf{z} = \text{softmax}(\mathbf{v}) \in \R^{C}$};
    \node [text width= 4cm, anchor = west] at (-1, \offset - 5.2*\d) {$\displaystyle z_i > 0,~~ \sum_{i=1}^C z_i = 1$};
   \end{scope}

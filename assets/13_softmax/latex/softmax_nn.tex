...
\begin{tikzpicture}[shorten >=1pt,draw=black,thick, node distance=\layersep, >= stealth]
   ...
   \begin{scope}[xshift = 7cm]
    ...
    \def\offset{-3.5}
    \def\d{.7}
    \node [text width= 4cm, anchor = west] at (-1, \offset + 3*\d) {$w_{0j}$: biases, don't forget!};
    ...
    \node [text width= 4cm, anchor = west] at (-1, \offset - 2*\d) {$z_i = \mathbf{w}_i^T\mathbf{x} $};
    \node [text width= 4cm, anchor = west] at (-1, \offset - 3*\d) {$\mathbf{z} = \mathbf{W}^T\mathbf{x} \in \R^{C}$};
    \node [text width= 4cm, anchor = west] at (-1, \offset - 4*\d) {$\mathbf{a} = \text{softmax}(\mathbf{v}) \in \R^{C}$};
    \node [text width= 4cm, anchor = west] at (-1, \offset - 5.2*\d) {$\displaystyle a_i > 0,~~ \sum_{i=1}^C a_i = 1$};
   \end{scope}

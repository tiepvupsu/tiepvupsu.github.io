...
Ở phần bên phải, hàm tuyến tính \\(\Sigma\\) và hàm softmax (activation function) được tách riêng ra để phục vụ cho mục đích minh họa. Dạng _short form_ ở bên phải là dạng hay được sử dụng trong các Neural Networks, lớp \\(\mathbf{a}\\) được ngầm hiểu là bao gồm cả lớp \\(\mathbf{v}\\).
...
Dưới đây là một đoạn code viết hàm softmax. Đầu vào là một ma trận với mỗi cột là một vector \\(\mathbf{v}\\), đầu ra cũng là một ma trận mà mỗi cột có giá trị là \\(\mathbf{a} = \text{softmax}(\mathbf{v})\\). Các giá trị của \\(\mathbf{v}\\) còn được gọi là **scores**.
...
Vậy một phương pháp đơn giản giúp khắc phục hiện tượng overflow là trừ tất cả các \\(z\_i\\) đi một giá trị đủ lớn. Trong thực nghiệm, giá trị đủ lớn này thường được chọn là \\(c = \max\_i z\_i\\). Vậy chúng ta có thể sửa đoạn code cho hàm `softmax` phía trên bằng cách trừ mỗi cột của ma trận đầu vào `V` đi giá trị lớn nhất trong cột đó. Ta có phiên bản ổn định hơn là `softmax_stable`:
...
\\[
\begin{eqnarray}
a\_1 &=& \frac{\exp(\mathbf{w}\_1\mathbf{x})} {\exp(\mathbf{w}\_1^T\mathbf{x}) + \exp(\mathbf{w}\_2^T\mathbf{x})} \\\

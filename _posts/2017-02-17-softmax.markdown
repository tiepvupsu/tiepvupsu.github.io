...
mathjax: true
tags: Neural-nets Supervised-learning Regression Multi-class
...
* Cột 4: Nếu \\(v\_1 = v\_2\\), thì \\(z\_1 = z\_2\\).

Bạn đọc có thể thử với các giá trị khác trực tiếp trên trình duyệt bằng trong [link này](http://neuralnetworksanddeeplearning.com/chap3.html), kéo xuống phần Softmax. 
...
Khi sử dụng mô hình Softmax Regression, với mỗi đầu vào \\(\mathbf{x}\\), ta sẽ có _đầu ra dự đoán_ là \\(\mathbf{z} = \text{softmax}(\mathbf{W}^T\mathbf{x})\\). Trong khi đó, _đầu ra thực sự_ chúng ta có là vector \\(\mathbf{y}\\) được biểu diễn dưới dạng one-hot coding. 
...
&=& u\_{ji}\mathbf{x}\_{i} ~(\text{where}~ u\_{ji} = z\_{ji} - v\_{ji}) ~~(5)
\end{eqnarray}
\\]

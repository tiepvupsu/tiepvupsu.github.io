...
img: /assets/29_lda/lda2.png
summary: PCA là phương pháp giảm chiều dữ liệu sao cho lượng thông tin về dữ liệu, thể hiện ở tổng phương sai, được giữ lại là nhiều nhất. Tuy nhiên, trong nhiều trường hợp, ta không cần giữ lại lượng thông tin lớn nhất mà chỉ cần giữ lại thông tin cần thiết cho riêng bài toán. LDA sẽ hiệu quả hơn với các bài toán classification.
---
**Trong trang này:**
...
&=&\mathbf{w}^T \underbrace{\sum_{k=1}^2 \sum_{n \in \mathcal{C}\_k} (\mathbf{x}\_n - \mathbf{m}\_k)(\mathbf{x}\_n - \mathbf{m}\_k)^T}_{\mathbf{S}_W} \mathbf{w} ~~~~~(6)
\end{eqnarray}
\\]
...
<table width = "100%" style = "border: 0px solid white">
    ...
    <tr >
        ...
        <img style="display:block;" width = "100%" src = "/assets/29_lda/res.png">
         ...
         <td width="40%" style = "border: 0px solid white" align = "center">
         Hình 4: Ví dụ minh hoạ về LDA trong không gian 2 chiều.
         </td>
      </tr>
</table>
</div>

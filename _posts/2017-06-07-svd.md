...
trong đó \\(\mathbf{I}\\) là ma trận đơn vị bậc \\(\mathbf{m}\\). Ta gọi \\(\mathbf{U}\\) là _ma trận trực giao_ (orthogonal matrix). _Ma trận loại này không không được gọi là ma trận trực chuẩn, không có định nghĩa cho ma trận trực chuẩn._

Một vài tính chất: 

1. \\(\mathbf{U}^{-1} = \mathbf{U}^T\\): nghịch đảo của một ma trận trực giao chính là chuyển vị của nó. 

2. Nếu \\(\mathbf{U}\\) là ma trận trực chuẩn thì chuyển vị của nó \\(\mathbf{U}^T\\) cũng là một ma trận trực chuẩn. 
...
4. Ma trận trực chuẩn thể hiện cho phép xoay (rotate) một vector. Giả sử có hai vector \\(\mathbf{x,y} \in \mathbb{R}^m\\) và ma trận trực chuẩn \\(\mathbf{U} \in \mathbb{R}^{m \times m}\\). Dùng ma trận này để xoay hai vector trên ta được \\(\mathbf{Ux}, \mathbf{Uy}\\). Tích vô hướng của hai vector mới là: 
\\[

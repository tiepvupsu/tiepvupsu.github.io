...
<hr>
 Nhận thấy rằng với các bài toán OR, AND, và OR, dữ liệu là [_linearly separable_](/2017/01/21/perceptron/#bai-toan-perceptron), vì vậy ta có thể tìm được các hệ số cho perceptron giúp biểu diễn chính xác mỗi hàm số. Xem ví dụ với hàm NOT, khi \\(x_1 = 0\\), ta có \\(z = \text{sgn}(-2 \times 0+1) = 1\\). Khi \\(x_1 = 1\\), \\(z = \text{sgn}(-2\times 1 + 1) = -1\\). Trong cả hai trường hợp, predicted output đều giống với [ground truth](/2017/01/08/knn/#ground-truth). Bạn đọc có thể tự kiểm chứng các hệ số trong hình với hàm AND và OR.
...
_Khi làm việc với những Neural Networks phức tạp, các tốt nhất để hạn chế lỗi là viết cụ thể chiều của mỗi ma trận hay vector ra, bạn sẽ thấy cụ thể hơn trong phần sau._
...
Trong đó \\(e_j^{(L)} = \frac{\partial J}{\partial z_j^{(L)}} \\) thường là một đạilượng _dễ tính toán_ và \\(\frac{\partial z_j^{(L)}}{\partial w_{ij}^{(L)}}  = a_i^{(L-1)}\\) vì \\(z_j^{(L)} = \mathbf{w}_j^{(L-1)T}\mathbf{a}^{(L-1)} + b_j^{(L-1)}\\).
...
Khi đó, cặp (input, output) sẽ ở dạng ma trận \\((\mathbf{X, Y})\\). Giả sử rằng mỗi lần tính toán, ta lấy \\(N\\) dữ liệu để tính toán. Khi đó, \\(\mathbf{X} \in \mathbb{R}^{d^{(0)} \times N}, \mathbf{Y} \in \mathbb{R}^{d^{(L)}\times N}\\). Với \\(d^{(0)} = d\\) là số chiều của dữ liệu đầu vào (không tính bias).
...
<div class="imgcap">
 ...
 <div class = "thecap">Hình : 2-layer Neural Networks.</div>
</div>
<hr>
...
Áp dụng [quy tắc như đã trình bày ở trên](/2017/02/24/mlp/#backpropagation-cho-batch-mini-batch-gradient-descent) và
...
\mathbf{E}^{(1)} &=& \left(\mathbf{W}^{(2)T}\mathbf{E}^{(2)}\right) \odot f'(\mathbf{Z}^{(1)}) \\\
...
<div class = "thecap"> Hình Kết quả với số lượng units trong hidden layer là khác nhau. </div>
</div>
...
* Với `d1 = 15`, mặc dù kết quả đã đạt 99.33%, vẫn có một vùng đỏ nhỏ nằm giữa nhánh màu lục và màu lam. Khi dữ liệu test rơi vào vùng này, nó sẽ bị phân loại sai.

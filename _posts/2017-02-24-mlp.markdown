...
- [Backpropagation](#backpropagation)
    - [Backpropagation cho Stochastic Gradient Descent](#backpropagation-cho-stochastic-gradient-descent)
        - [Đạo hàm theo từng hệ số \\\(w_{ij}^{\(l\)}, b_{i}^{\(l\)}\\\)](#dao-ham-theo-tung-he-so-\\wij^l-bi^l\\)
        - [Đạo hàm theo ma trận \\\(\mathbf{W}^{\(l\)}, \mathbf{b}^{\(l\)}\\\)](#dao-ham-theo-ma-tran-\\\mathbfw^l-\mathbfb^l\\)
    - [Backpropagation cho Batch \(mini-batch\) Gradient Descent](#backpropagation-cho-batch-mini-batch-gradient-descent)
- [Ví dụ trên Python](#vi-du-tren-python)
    - [Tạo dữ liệu giả](#tao-du-lieu-gia)
...
Bây giờ chúng ta sẽ áp dụng Batch Gradient Descent cho bài toán này. Trước hết cần thực tìm công thức tính các activation và output.
...
\\]
(Tôi đã cho thêm thừa số \\(\frac{1}{N}\\) để tránh hiện tượng tổng quá lớn với Batch GD. Về mặt toán học, thừa số này không làm thay đổi nghiệm của bài toán)
...
\\[
\begin{eqnarray}
\mathbf{E}^{(2)} &=& \frac{\partial J}{\partial \mathbf{Z}^{(2)}} = \mathbf{\hat{Y}} - \mathbf{Y} \\\
...
\frac{\partial J}{\partial \mathbf{W}^{(1)}} &=& \mathbf{A}^{(1)}  \mathbf{E}^{(1)T} \\\
\frac{\partial J}{\partial \mathbf{b}^{(1)}} &=& \sum_{n=1}^N\mathbf{e}\_n^{(1)} \\\
\end{eqnarray}
\\]

Từ đó ta có chương trình như sau:


<hr>
<div class="imgcap">
 <img src ="/assets/14_mlp/ex_res.png" align = "center" width = "600">
 <div class = "thecap">Hình : Phân bố dữ liệu theo class.</div>
</div>
<hr>

...
layout: post
comments: true
title:  "Bài 30: Ôn tập lại Xác Suất"
title2:  "30. Probability"
...
summary: Ôn tập lại về Xác Suất và các Phân phối thường dùng.
---
**Trong trang này:**
...
3. Bài 9-15: Neural Networks. Phần này giúp các bạn nắm được các thành phần cơ bản của một Neural Network và các Networks cơ bản nhất, chủ yếu cho các bài toán classification. Các bài toán tối ưu đều là không ràng buộc, các hàm mất mát đều là không lồi, nghiệm tìm được là các local optimal.
...
\\[
\text{if}~ x ~\text{is discrete:}\quad \sum_{x} Pr(x) = 1 ~~~~ (1)
\\]

Biến ngẫu nhiên liên tục lấy các giá trị là tập con của các số thực. Những giá trị này có thể là hữu hạn, ví dụ thời gian làm bài của mỗi thí sinh trong một bài thi 180 phút, hoặc vô hạn, ví dụ thời gian để chiếc xe bus tiếp theo tới. Không như biến ngẫu nhiên rời rạc, xác suất để đầu ra bằng _chính xác_ một giá trị nào đó, theo lý thuyết, là bằng 0. Thay vào đó, ta có thể hình dung xác suất để đầu ra nằm trong một khoảng giá trị nào đó; và việc này được mô tả bởi _hàm mật đô xác suất_ (probability density function - pdf). Hàm mật độ xác suất luôn cho giá trị dương, và tích phân của nó trên toàn miền _posible outcome_ phải bằng 1.

\\[
\text{if}~ x ~\text{is continuous:}\quad \int Pr(x)dx = 1 ~~~~ (2)
\\]
Để giảm thiểu ký hiệu, hàm mật độ xác suất của một biến ngẫu nhiên liên tục \\(x\\) cũng được ký hiệu là \\(Pr(x)\\).

<a name="-joint-probability"></a>

### 2.2. Joint probability
Xét hai biến ngẫu nhiên \\(x\\) và \\(y\\). Nếu ta quan sát rất nhiều cặp đầu ra của \\(x\\) và \\(y\\), thì có những tổ hợp hai đầu ra xảy ra thường xuyên hơn những tổ hợp khác. Thông tin này được biểu diễn bằng một phân phối được gọi là _joint probability_ của \\(x\\) và \\(y\\), và được viết là \\(Pr(x, y)\\). Dấu phẩy trong \\(Pr(x, y)\\) có thể đọc là _và_, vậy \\(Pr(x, y)\\) là xác suất của \\(x\\) _và_ \\(y\\). \\(x\\) và \\(y\\) có thể là hai biến ngẫu nhiên rời rạc, liên tục, hoặc một rời rạc, một liên tục. Luôn nhớ rằng tổng các xác suất trên mọi cặp giá trị có thể xảy ra \\((x, y)\\) bằng 1.

\\[
\begin{eqnarray}
\text{both are discrete} \quad \quad & \sum_{x, y} Pr(x, y) &=& 1 \\\
\text{both are continuous} & \int Pr(x, y) dx dy &=& 1\\\
x ~\text{is discrete}, ~ y ~\text{is continuous} & \sum_{x} \int Pr(x, y) dy = \int \left\(\sum_{x} Pr(x, y) \right\)dy &=& 1
\end{eqnarray}
\\]

Xét ví dụ trong Hình 1, phần có nền màu lục nhạt. \\(x\\) là biến ngẫu nhiên chỉ điểm thi môn Toán của học sinh trong một trường trong kỳ thi Quốc gia, \\(y\\) là biến ngẫu nhiên chỉ điểm thi môn Vật Lý cũng trong kỳ thi đó. \\(Pr(x = x^\*, y = y^\*)\\) là tỉ lệ giữa tần suất số học sinh được \\(x^\*\\) điểm trong môn Toán và \\(y^\*\\) điêm trong môn Vật Lý và toàn bộ số học sinh. Tỉ lệ này có thể coi là xác suất khi số học sinh trong trường là lớn. Ở đây \\(x^\*\\) và \\(y^\*\\) là các số xác định. Thông thường, xác suất này được viết gọn lại thành \\(Pr(x^\*, y^\*)\\), và \\(Pr(x, y)\\) được dùng như một hàm tổng quát để mô tả các xác suất. Giả sử thêm rằng điểm các môn là các số tự nhiên từ 1 đến 10.

Các ô vuông màu lam thể hiện xác suất \\(Pr(x, y)\\) với diện tích ô vuông càng to thể hiện xác suất đó càng lớn. Chú ý rằng tổng các xác suất này bằng 1.
...
Thông thường, chúng ta sẽ làm việc với các bài toán ở đó joint probability xác định trên nhiều hơn 2 biến ngẫu nhiên. Chẳng hạn, \\(Pr(x, y, z)\\) thể hiện joint probability của 3 biến ngẫu nhiên \\(x,y\\) và \\(z\\). Khi có nhiều biến ngẫu nhiên, ta có thể viết chúng dưới dạng vector. Ta có thể viết \\(Pr(\mathbf{x})\\) để thể hiện joint probability của biến ngẫu nhiên nhiều chiều \\(\mathbf{x} = [x_1, x_2, \dots, x_n]^T\\). Khi có nhiều tập các biến ngẫu nhiên, ví dụ \\(\mathbf{x}\\) và \\(\mathbf{y}\\), ta có thể biết \\(Pr(\mathbf{x}, \mathbf{y})\\) để thể hiện joint probability của tất cả các thành phần trng hai biến ngẫu nhiên nhiều chiều này.
...
\\[
\begin{eqnarray}
  Pr(x) &=& \sum_{y}Pr(x, y) \quad & (3) \\\
  Pr(y) &=& \sum_{x}Pr(x, y) & (4)
\end{eqnarray}
\\]
Và với biến liên tục:
\\[
\begin{eqnarray}
  Pr(x) &=& \int Pr(x, y)dy \quad & (5)\\\
  Pr(y) &=& \int Pr(x, y)dx & (6)
\end{eqnarray}
\\]
...
\\[
\begin{eqnarray}
  Pr(x) &=& \sum_{ y, z, w}Pr(x, y, z, w) \quad & (7) \\\
  Pr(x, y) &=& \sum_{z, w}Pr(z, w) & (8)
\end{eqnarray}
\\]
...
Xác suất có điều kiện (conditional probaility) của một biến ngẫu nhiên \\(x\\) biết rằng biến ngẫu nhiên \\(y\\) có giá trị \\(y^\*\\) được ký hiệu là \\(Pr(x\| y = y^\*)\\) (đọc là _probability of \\(x\\) given that \\(y\\) takes value \\(y^\*\\)_).

Conditional probability \\(Pr(x \| y = y^\*)\\) có thể được tính dựa trên joint probobability \\(Pr(x, y)\\). Quay lại Hình 1 với vùng có nền màu nâu nhạt. Nếu biết rằng \\(y = 9\\), xác suất \\(Pr(x \| y = 9)\\) có thể tính được dựa trên hàng thứ 9 của hình vuông trung tâm, tức hàng \\(Pr(x, y = 9)\\). Trong hàng này, những ô vuông lớn hơn thể hiện xác suất lớn hơn. Tương ứng như thế, \\(Pr(x \| y = 9) \\) cũng lớn nếu \\(Pr(x, y= 9)\\) lớn. Chú ý rằng tổng các xác suất \\(\sum_{x} Pr(x, y = 9)\\) nhỏ hơn 1 và bằng tổng các xác suất trên hàng thứ 9 này. Để có _đúng_ xác suất, tức tổng các xác suất có điều kiện bằng 1, ta cần chia mỗi đại lượng \\(Pr(x, y = 9)\\) cho tổng của toàn hàng này. Tức là:

\\[
\displaystyle
  Pr(x \| y = 9) = = \frac{Pr(x, y = 9)}{\sum_x Pr(x, y = 9)} = \frac{Pr(x, y = 9)}{Pr(y = 9)}
\\]
Tổng quát:
\\[
\displaystyle
  Pr(x\|y = y^\*) = \frac{Pr(x, y = y^\*)}{\sum_{x} Pr(x, y = y^\*)} = \frac{Pr(x, y = y^\*)}{Pr(y = y^\*)}  ~~~~ (9)
\\]
...
và ta sẽ có quan hệ:
\\[
  Pr(x, y) = Pr(x\|y)Pr(y) = Pr(y \| x) Pr(x) ~~~ (11)
\\]
...
\begin{eqnarray}
  Pr(x, y, z, w)
  & = & Pr(x, y, z \| w) Pr(w) & (12)\\\
  & = & Pr(x, y \| z, w)Pr(z, w) = Pr(x, y \| z, w) Pr(z \| w) Pr(w) \quad & (13) \\\
  & = & Pr(x | y, z, w)Pr(y | z, w) Pr(y | w) Pr(w) & (14)
\end{eqnarray}
\\]
...
### 2.5. Quy tắc Bayes
Công thức \\((11)\\) biểu diễn joint probability theo hai cách. Từ đây ta có thể suy ra quan hệ giữa hai conditional probabilities \\(Pr(x \|y)\\) và \\(Pr(y \| x)\\):
\\[
...
\begin{eqnarray}
  Pr(y \| x)
  & = & \frac{Pr(x \|y) Pr(y)}{Pr(x}  & (15)\\\
  & = & \frac{Pr(x \|y) Pr(y)}{\sum_{y} Pr(x, y)} & (16)\\\
  & = & \frac{Pr(x \|y) Pr(y)}{\sum_{y} Pr(x \| y) Pr(y)} \quad & (17)
\end{eqnarray}
\\]
ở đó dòng thứ hai và ba đã sử dụng các công thức về marginal và conditional probability ở mẫu số. Từ \\((17)\\) ta có thể thấy rằng \\(Pr(y \| x)\\) hoàn toàn có thể tính được nếu ta biết mọi \\(Pr(x \| y)\\) và \\(Pr(y)\\). Tuy nhiên, việc tính trực tiếp xác suất này thường là phức tạp. Thay vào đó, ta có thể đi tìm mô hình phù hợp của \\(Pr(\mathbf{x} \| y)\\) trên training data sao cho _những gì đã thực sự xảy ra có xác suất cao nhất có thể_ (Chỗ này có thể hơi khó hình dung, tôi sẽ nói cụ thể về việc này trong bài tiếp theo). Dựa trên training data, các tham số của mô hình này có thể tìm được qua một bài toán tối ưu.

**Ba công thức \\((15) - (17)\\) thường được gọi là _Quy tắc Bayes_ (Bayes' rule). Quy tắc này rất quan trọng trong Machine Learning!**

Trong Machine Learning, chúng ta thường mô tả quan hệ giữa hai biến \\(x\\) và \\(y\\) dưới dạng xác suất có điều kiện \\(Pr(x\|y)\\). Ví dụ, biết rằng đầu vào là một bức ảnh ở dạng vector \\(\mathbf{x}\\), xác suất để bức ảnh chứa một chiếc xe là bao nhiêu. Khi đó, ta phải tính \\(Pr(y \| \mathbf{x})\\).
...
\\[
  Pr(x, y) = Pr(x \| y) Pr(y) = Pr(x) Pr(y) ~~~ (20)
\\]
...
\\[
\begin{eqnarray}
  \text{E}[x] = \sum_x x Pr(x) \quad & \text{if}~ x ~ \text{is discrete} \quad & (21)\\\
  \text{E}[x] = \int x Pr(x) dx \quad & \text{if}~ x ~ \text{is continuous} & (22)
\end{eqnarray}
\\]
...
\\[
  \text{E}[f(x, y)] = \sum_{x,y} f(x, y) Pr(x, y) dx dy  ~~~ (24)
\\]
...
\\[
  Pr(x = 1) = \lambda, ~~~~ Pr(x = 0) = 1 - Pr(x = 1) = 1 - \lambda ~~~ (29)
\\]

Hai đẳng thức này thường được viết gọn lại:
\\[
  Pr(x) = \lambda^x (1 - \lambda)^{1 - x} ~~~~ (29)
\\]
...
\\[
Pr(\mathbf{x} = \mathbf{e}\_k) = \prod_{j=1}^K \lambda_j^{x_j} = \lambda_k ~~~~ (31)
\\]
...
\\[
  Pr(x) = \frac{1}{\sqrt{2\pi \sigma^2}}\exp \left\( -\frac{(x - \mu)^2}{2\sigma^2}\right\)~~~~ (32)
\\]
Dạng gọn hơn:
\\[
...
\\[
  Pr(\mathbf{x}) = \frac{1}{(2\pi)^{D/2} \|\Sigma\|^{1/2}} exp \left\(\frac{1}{2} (\mathbf{x} - \mu) \Sigma^{-1} (\mathbf{x} - \mu)\right\) ~~~ (33)
\\]
với \\(\|\Sigma\|\\) là định thức của ma trận hiệp phương sai \\(\Sigma\\).

Hoặc viết gọn:
\\[
  Pr(\mathbf{x}) = \text{Norm}\_{\mathbf{x}}[\mu, \Sigma]
\\]
...
\\[
  Pr(\lambda) = \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha) \Gamma(\beta)} \lambda^{\alpha - 1} ( 1 - \lambda) ^{\beta - 1} ~~~ (34)
\\]
...
Hàm mật độ xác suất:
\\[
  Pr(\lambda_1, \dots, \lambda_K) = \frac{\Gamma(\sum_{k=1}^K \alpha_k)}{\prod_{k=1}^K\Gamma(\alpha_k)} \prod_{k=1}^K \lambda_k^{\alpha - 1}~~~ (34)
\\]

Viết gọn:
\\[
  Pr(\lambda_1, \dots, \lambda_K) = \text{Dir}_{\lambda_1, \dots, \lambda_K}[\alpha_1, \dots, \alpha_K]
\\]

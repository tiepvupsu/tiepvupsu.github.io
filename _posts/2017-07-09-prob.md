...
Xác suất có điều kiện (conditional probaility) của một biến ngẫu nhiên \\(x\\) biết rằng biến ngẫu nhiên \\(y\\) có giá trị \\(y^\*\\) được ký hiệu là \\(p(x\| y = y^\*)\\) (đọc là _probability of \\(x\\) given that \\(y\\) takes value \\(y^\*\\)_).
...
\begin{eqnarray}
  ...
  & = & p(x | y, z, w)p(y | z, w) p(y | w) p(w) & (14)
\end{eqnarray}
\\]
...
Hàm mật độ xác suất:
\\[
  p(\lambda_1, \dots, \lambda_K) = \frac{\Gamma(\sum_{k=1}^K \alpha_k)}{\prod_{k=1}^K\Gamma(\alpha_k)} \prod_{k=1}^K \lambda_k^{\alpha - 1}~~~ (34)
\\]
